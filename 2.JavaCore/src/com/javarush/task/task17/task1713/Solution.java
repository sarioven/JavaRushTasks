...
public class Solution implements List<Long> {
    ...
    public static void main(String[] args) {
        ...
        String s = "наша строка";   // Записываем нашу строку
        String newS = "";           // Готовим переменную под новую строку
        boolean toUpper =  true;    // Переменная, говорящая о том, что буква является началом слова

        for(char ss : s.toCharArray()) { // Разбиваем нашу строку на символы и проходимся по каждому
            if(toUpper) {                // Добавляем новый символ в зависимости от ситуации
                newS += "" + Character.toUpperCase(ss); // Заглавная
                toUpper = false;
            }
            else
                newS += "" + ss; // Обычная

            if(ss == ' ') // Если символ пробел, то говорим программе, что следющий символ будет началом слова
                toUpper = true;
        }

        System.out.println(newS); // выводим новую строку
    }
